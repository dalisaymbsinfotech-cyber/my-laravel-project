...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sidebar { width: var(--sidebar-width); background: var(--red); min-height: 100vh; position: fixed; top: 0; left: 0; display: flex; flex-direction: column; z-index: 100; box-shadow: 2px 0 10px rgba(0,0,0,0.2); }
    .sidebar-header { padding: 20px 15px; border-bottom: 1px solid rgba(255,255,255,0.1); display: flex; align-items: center; gap: 10px; }
    .sidebar-header img { width: 45px; height: 45px; object-fit: contain; border-radius: 6px; }
    ...
    .sidebar-nav a { display: flex; align-items: center; gap: 12px; padding: 12px 20px; color: rgba(255,255,255,0.8); text-decoration: none; font-size: 14px; transition: all 0.2s; border-left: 3px solid transparent; }
    .sidebar-nav a:hover, .sidebar-nav a.active { background: rgba(255,255,255,0.1); color: white; border-left: 3px solid var(--gold); }
    .sidebar-nav a .icon { font-size: 18px; width: 24px; text-align: center; }

    .sidebar-footer { padding: 15px; border-top: 1px solid rgba(255,255,255,0.1); display: flex; flex-direction: column; gap: 4px; }
    .sidebar-footer a { display: flex; align-items: center; gap: 10px; color: rgba(255,255,255,0.7); text-decoration: none; font-size: 14px; padding: 10px; border-radius: 6px; transition: all 0.2s; }
    ...
    .topbar .topbar-right span { color: #666; font-size: 13px; }
    ...
    .btn { padding: 10px 20px; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-family: 'Poppins', sans-serif; font-weight: 500; transition: all 0.2s; }
    .btn-primary { background: var(--red); color: white; }
    .btn-primary:hover { background: var(--red-light); }
    .btn-gold { background: var(--gold); color: var(--red); font-weight: 600; }
    ...
    .badge-on_time { background: #d4edda; color: #155724; }
    .badge-late { background: #fff3cd; color: #856404; }
    .badge-absent { background: #f8d7da; color: #721c24; }

    .modal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); justify-content: center; align-items: center; z-index: 1000; }
    .modal.active { display: flex; }
    .modal-box { background: white; padding: 30px; border-radius: 10px; width: 500px; border-top: 4px solid var(--red); max-height: 90vh; overflow-y: auto; }
    .modal-box h3 { color: var(--red); margin-bottom: 15px; }
...
<div class="sidebar" id="sidebar">
    <div class="sidebar-header">
        ...
        <div>
            ...
            <span>{{ $settings->system_name ?? 'School Admin System' }}</span>
        </div>
    </div>
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('admin.colleges') }}" class="{{ request()->routeIs('admin.colleges') ? 'active' : '' }}">
            <span class="icon">🏛️</span> Colleges
        </a>
        <a href="{{ route('admin.departments') }}" class="{{ request()->routeIs('admin.departments') ? 'active' : '' }}">
            <span class="icon">🎓</span> Departments
        </a>
        <a href="{{ route('admin.classrooms') }}" class="{{ request()->routeIs('admin.classrooms*') ? 'active' : '' }}">
            <span class="icon">🚪</span> Classrooms
        </a>
        <a href="{{ route('admin.sections') }}" class="{{ request()->routeIs('admin.sections*') ? 'active' : '' }}">
        ...
        <a href="{{ route('admin.schedules') }}" class="{{ request()->routeIs('admin.schedules') ? 'active' : '' }}">
            <span class="icon">🗓️</span> Schedules
        </a>
        ...
        <a href="{{ route('admin.enrollment') }}" class="{{ request()->routeIs('admin.enrollment') ? 'active' : '' }}">
            <span class="icon">📝</span> Enrollment
        </a>

        <div class="nav-label">Staff</div>
        <a href="{{ route('admin.professors') }}" class="{{ request()->routeIs('admin.professors') ? 'active' : '' }}">
            <span class="icon">👨‍🏫</span> Manage Professors
        </a>
    ...
    <div class="sidebar-footer">
        <a href="{{ route('admin.settings') }}" class="{{ request()->routeIs('admin.settings') ? 'active' : '' }}" style="{{ request()->routeIs('admin.settings') ? 'background:rgba(255,255,255,0.1); color:white;' : '' }}">
            <span>⚙️</span> System Settings
        </a>
        <form method="POST" action="{{ route('logout') }}" style="margin:0;">
            @csrf
            <button type="submit" style="display:flex; align-items:center; gap:10px; width:100%; background:none; border:none; color:rgba(255,255,255,0.7); font-size:14px; padding:10px; border-radius:6px; cursor:pointer; font-family:inherit; text-align:left;">
                <span>🚪</span> Logout
            </button>
        </form>
    </div>
