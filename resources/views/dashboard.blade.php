...
<div class="top-bar">
    <div>
        ...
        <p>Welcome back! Here's your school system overview.</p>
    </div>
    <div class="search-bar">
        <input type="text" placeholder="Search students, classrooms..." id="searchInput" oninput="filterData()" />
...
<div class="metric-grid">
    ...
    <div class="metric-card">
        ...
        <div class="value">{{ $totalFaces ?? 0 }}</div>
        <div class="change">Face records captured</div>
    </div>
    <div class="metric-card">
        <div class="label">Professors</div>
        <div class="value">{{ $totalProfessors ?? 0 }}</div>
        <div class="change">Active professors</div>
    </div>
    <div class="metric-card">
        <div class="label">Classrooms</div>
        <div class="value">{{ $totalRooms ?? 0 }}</div>
        <div class="change">Available classrooms</div>
    </div>
    <div class="metric-card">
        <div class="label">Subjects</div>
        <div class="value">{{ $totalSubjects ?? 0 }}</div>
        <div class="change">Active subjects</div>
    </div>
    <div class="metric-card">
        <div class="label">Enrolled</div>
        <div class="value">{{ $totalEnrollments ?? 0 }}</div>
        <div class="change">Total enrollments</div>
    </div>
    <div class="metric-card">
        <div class="label">Schedules</div>
        <div class="value">{{ $totalSchedules ?? 0 }}</div>
        <div class="change">Active schedules</div>
    </div>
</div>
...
<div class="chart-grid">
    <div class="chart-card">
        ...
        <p>Enrollment count by section (from enrollments)</p>
        <div class="legend">
    ...
    <div class="chart-card">
        ...
        <p>Present / Late / Absent for Mon–Fri this week ({{ $attendanceWeekLabel ?? '' }})</p>
        <div class="legend">
...
<div class="full-chart">
    ...
    <p>New enrollment records created per month (academic year Aug–{{ now()->format('M Y') }})</p>
...
<div class="info-grid">
    <div class="info-card">
        ...
        <div id="enrollment-list">
            ...
            @empty
            <div class="info-row">
                <div class="meta">No recent enrollments yet.</div>
            </div>
            @endforelse
        </div>
    </div>
    <div class="info-card">
        ...
        @empty
        <div class="info-row">
            <div class="meta">No classrooms added yet.</div>
        </div>
        @endforelse
    </div>
</div>
    ...
    options: {
        ...
        plugins: {
            ...
            tooltip: {
                callbacks: {
                    label: function(ctx) {
                        if (sectionEmpty) return ' Add enrollments to see breakdown';
                        const v = ctx.raw;
